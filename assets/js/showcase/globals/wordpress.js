...
( function( $, window, undefined ) {
	...
	window.columns = {
		...
		colSpanChange: function( diff ) {
			let $t = $( 'table' ).find( '.colspanchange' ),
	n;
			if ( ! $t.length ) {
	return;
	}
	...
	window.showNotice = {
		 ...
		 */
		warn: function() {
			let msg = 'You are about to permanently delete these items from your site. This action cannot be undone. \'Cancel\' to stop, \'OK\' to delete.' || '';
			if ( confirm( msg ) ) {
				return true;
			}
	 ...
	 */
	let permalinkStructureFocused = false,
	...
	$availableStructureTags.on( 'click', function() {
		let permalinkStructureValue = $permalinkStructure.val(),
	...
	$document.ready( function() {
		let checks, first, last, checked, sliced, mobileEvent, transitionTimeout, focusedRowActions,
			lastClicked = false,
		...
		function currentMenuItemHasPopup() {
			let $current = $( 'a.wp-has-current-submenu' );
		...
		if ( 'ontouchstart' in window || /IEMobile\/[1-9]/.test( navigator.userAgent ) ) { // touch screen device
			...
			$adminmenu.find( 'a.wp-has-submenu' ).on( mobileEvent + '.wp-mobile-hover', function( event ) {
				...
				if ( ! $menuItem.hasClass( 'opensub' ) && ( ! $menuItem.hasClass( 'wp-menu-open' ) || 40 > $menuItem.width() ) ) {
		...
		if ( ! isIOS && ! isAndroid ) {
			 ...
			 * @returns {void}
			 */
			$adminmenu.find( 'li.wp-has-submenu' ).on( 'mouseover', function() {
				...
				if ( $adminmenu.data( 'wp-responsive' ) ) { // The menu is in responsive mode, bail
					return;
				}
	...
	return checked;
			...
			let unchecked = $( this ).closest( 'tbody' ).find( ':checkbox' ).filter( ':visible:enabled' ).not( ':checked' );
	...
	e.preventDefault();
		...
		if ( pageInput.length ) {
			...
			pageInput.closest( 'form' ).submit( function() {
				...
				*/
				if ( -1 == $( 'select[name="action"]' ).val() && -1 == $( 'select[name="action2"]' ).val() && pageInput.val() == currentPage ) {
	pageInput.val( '1' );
	}
			});
		}
	...
	return;
		...
		function pinMenu( event ) {
			let windowPos = $window.scrollTop(),
				resizing = ! event || 'scroll' !== event.type;
		...
		*/
		window.wpResponsive = {
			...
			maybeDisableSortables: function() {
				let width = -1 < navigator.userAgent.indexOf( 'AppleWebKit/' ) ? $window.width() : window.innerWidth;
		...
		function setMenuState() {
			...
			} else if ( $body.hasClass( 'folded' ) || ( $body.hasClass( 'auto-fold' ) && 960 >= viewportWidth && 782 < viewportWidth ) ) {
				menuState = 'folded';
			} else {
				menuState = 'open';
			}
		...
		$body.on( 'click', '.js-update-details-toggle', function() {
			let $updateNotice = $( this ).closest( '.js-update-details' ),
