...
library.watch.styles = [library.source.styles + "**/**/*.scss"];

library.source.scripts = "./assets/js/shared-ui/";
library.output.scripts = "./_dist/library/dist/js/";

library.watch.scripts = [library.source.scripts + "**/**/*.js"];
...
showcase.watch.styles = [showcase.source.styles + "**/*.scss"];

showcase.source.scripts = "./assets/js/showcase/";
showcase.output.scripts = "./assets/js/";

showcase.watch.scripts = [showcase.source.scripts + "**/**/*.js"];
...
function watch() {
	...
	gulp.watch(library.watch.styles, suiCss);
	...
	gulp.watch(library.watch.scripts, suiJs);
...
function updateConfigVersion() {
	...
	return gulp
		.src("./_config.yml")
		.pipe(
			replace(/(suiver: ').*(')/gm, function (match, p1, p2) {
				console.log(chalk.magentaBright("./_config.yml:"));
				console.log(
					`${chalk.green("suiver")} has been updated to ${chalk.green(
						`${version}`
					)}\n`
				);
		...
		.pipe(
			replace(/(suiclass: ').*(')/gm, function (match, p1, p2) {
				console.log(chalk.magentaBright("./_config.yml:"));
				console.log(
					`${chalk.green(
						"suiclass"
					)} suiclass has been updated to ${chalk.green(bodyClass)}\n`
				);
...
function updateVariablesVersion() {
	...
	return gulp
		.src(library.source.styles + "_variables.scss")
		.pipe(
			replace(/^(\$sui-version: ).*(;)/gm, function (match, p1, p2) {
				console.log(chalk.magentaBright("\n./scss/_variables.scss:"));
				console.log(
					`${chalk.green("$sui-version")} has been updated to ${chalk.green(
						version
					)}\n`
				);
...
function copyPreBuiltJs() {
	return gulp.src(library.watch.scripts).pipe(gulp.dest("./_dist/library/js/"));
}
...
function copyPages() {
	return gulp
		.src(["./*.html", "_config.yml", ".gitignore"])
		.pipe(gulp.dest("./_dist/showcase/"));
}

// Showcase layouts.
function copyLayouts() {
	return gulp.src("./_layouts/*").pipe(gulp.dest("./_dist/showcase/_layouts/"));
}
...
exports.update = gulp.series(updateConfigVersion, updateVariablesVersion);
exports.copy = gulp.series(
	copyMainLibrary,
	copySuiFont,
	copyDashicons,
	copyImages,
	gulp.parallel(copyPreBuiltCss, copyPreBuiltJs),
	gulp.parallel(copyCss, copyJs, copyPages, copyLayouts, copyComponents)
);
